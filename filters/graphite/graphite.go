...
var metricExp = GraphiteMetricExp{regexp.MustCompile(`^[\s]*(?:put)?[\s]*(?P<datacenter>[\w]+)\.(?P<host>[^_]+)_(?P<domain>[\w]+).(?P<service_type>[A-Za-z\-]+)(?P<service_id>[0-9]{3})?(?P<source>[a-z\_\d]+)?.(?P<component>[\w]+).(?P<metric>.*)$`)}
